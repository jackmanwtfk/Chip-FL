...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_experiment_design.sh [--group all|exp1|exp2] [--dry-run]
EOF
}
...
MANIFEST="configs/exp_design_ls+qs/manifest.json"
if [[ ! -f "$MANIFEST" ]]; then
...
group = sys.argv[1]
items = json.loads(Path("configs/exp_design_ls+qs/manifest.json").read_text(encoding="utf-8"))
for it in items:
    if group == "all":
...
echo "[INFO] Selected group=$GROUP, cases:"
printf '%s\n' "${CASE_LINES[@]}"
...
LOG_DIR="results/exp_design_logs/${RUN_TAG}_${GROUP}"
mkdir -p "$LOG_DIR"
...
echo "[INFO] Experiment-design runs completed."
